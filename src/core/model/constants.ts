...
/* ------------------------------------------------------------
 * v0.14：個人資産の収支・結婚市場・配偶者インカム
 * ------------------------------------------------------------ */
// PC役員報酬（会社CASH→個人資産wealth・毎ターン）。控えめ＝両業態の黒字化/生存を維持。
export const PC_SALARY_BASE = 400;      // 基礎役員報酬（×lifestyleFactor）
export const LIFESTYLE_COST_BASE = 250; // 個人の生活費（×lifestyleFactor・wealthから毎ターン）
// 配偶者インカム（有能・高評判な伴侶ほど稼ぐ＝会社CASHではなく世帯収入としてwealthへ）
export const SPOUSE_INCOME_K = 6;       // 配偶者インカム係数（×(CA/10 + reputation/10)）
// 結婚市場（人材Derとは別の専用プール）
