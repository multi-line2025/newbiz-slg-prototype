...
export const AP_COST = {
  hire: 1, // 採用オファー（v0.11：オファー提出＝1AP。着任は3ターン後の受諾判定）
  assign: 1, // 役割配属
...
/* ============================================================
 * v0.11：リクルート（3ターンのオファー→受諾判定）
 *  採用は即時ではなくオファー方式。情報リーク（PA/評判上限の露出）を断ち、
 *  高位人材は無名企業には確率的に辞退する（閾値・PA数値は一切出さない）。
 * ============================================================ */
export const RECRUIT_TURNS = 3; // オファー提出から返答までのターン数
export const MAX_PENDING_OFFERS = 3; // 同時進行オファーの上限（mass-probe抑止の“枠”）
// 受諾確率の内部係数（UIには一切露出しない）。
export const OFFER_PA_MARGIN_DIV = 40; // 到達上限PAとの差をこの幅で正規化（境界をぼかす）
export const OFFER_BASE = 0.75;        // 評判で届く範囲(PA<=上限)の基準受諾率
export const OFFER_SALARY_K = 0.5;     // 提示給与/要求給与 −1 の感度（高提示ほど受諾↑）
export const OFFER_AMBITION_K = 0.2;   // 野心×(無名度)で受諾↓（野心家は無名を敬遠）
export const OFFER_ACCEPT_MIN = 0.02;  // 受諾確率の下限（境界付近も0にしない＝上限特定を困難に）
export const OFFER_ACCEPT_MAX = 0.98;  // 受諾確率の上限
...
/* ------------------------------------------------------------
 * v0.10：単一ワールド人材DB（約500人・全ティア混在）
 *  ユーザー確定：DBの大半は普通/低スキル、稀にエリート。labor/knowledge で別プールにしない。
 *  低ティア（未熟練 PA<80）を世界に含めることで v0.9 の「安い頭数」を統合的に表現する。
 *  2つのゲートは別軸：REPUTATION_GATES＝採用可否（存在はする／評判が要る）、
 *  スカウトサブスク＝可視性（見えるか）。→ 生成時にはゲートで除外しない（全ティアが存在）。
 * ------------------------------------------------------------ */
export const WORLD_POOL_SIZE = 500; // 単一ワールドDBの人数（seed再現・軽量）
export const WORLD_PA_TIERS: PaTier[] = [
  { min: 45, max: 80, ratio: 0.34, label: "未熟練" },   // 大半：安い低スキル（labor の頭数）
  { min: 80, max: 120, ratio: 0.45, label: "一般" },    // 普通の人材
  { min: 120, max: 150, ratio: 0.14, label: "優秀" },
  { min: 150, max: 170, ratio: 0.048, label: "一流" },
  { min: 170, max: 185, ratio: 0.02, label: "超一流" },
  { min: 185, max: 200, ratio: 0.002, label: "世代の逸材" },
];
export const UNSKILLED_PA_MAX = 80; // これ未満＝未熟練。若年寄せで低CA＋stamina/health確保
export const ORDINARY_PA_MIN = 80;  // knowledge開始選抜が拾う「普通の人材」の下限

/* ------------------------------------------------------------
 * v0.10：国別スカウトサブスク（月額・可視性ゲート）
 *  加入国は候補者の★(occStars概要)が見える。未加入国は一覧に出さず素性も不明。
 *  月額は市場規模/GDPで差（$500〜$2,000）。継続コストは monthlyBurn に加算、いつでも解約可。
 *  起業国(home)は開始時から加入済み。
 * ------------------------------------------------------------ */
export const SCOUT_SUB_COST: Record<PlayableCountry, number> = {
...
/* ============================================================
 * v0.13：個人キャリア＆家族システム（恋愛・結婚・出産・教育）
 *  §9.3 出産／§9.3.1 妊孕性／§9.3.3 血族婚／§9.4 教育／§10 後継。
 *  独立サブシステム：既存の経営ロジック（収支・QUAL・市場）には影響させない。
 * ============================================================ */
// 恋愛/結婚の評判釣り合いゲート（★最重要）。|rep_PC − rep_相手| がこの幅を超えると不可。
export const REP_MATCH_MAX = 35; // 評判差の許容上限（超で成立確率0＝格差ありすぎ）
export const COURT_BASE = 0.6;   // 求愛の基礎成功率（×評判釣り合い）
export const PROPOSE_BASE = 0.55; // 求婚の基礎成功率（×評判釣り合い）
export const MARRIAGE_LUMP = 10000; // 結婚一時金（個人資産WEALTHから・§12.5）
export const ROMANCE_MIN_AGE = 18;  // 恋愛/結婚の成人前提
// 妊娠・出産（§9.3）
export const CONCEIVE_BASE = 0.35; // 毎ターンの受胎ベース率（×父妊孕性×母妊孕性×健康補正）
export const GESTATION_TURNS = 12; // 妊娠期間（1ターン=1ヶ月 → 約1年で出産）
export const PA_MUTATION = 12;      // 子PA＝両親平均±この範囲の突然変異（§9.3）
// 教育（§9.4）：子に個人資産/APを投じて成長を加速。
export const EDU_COST = 3000;       // 教育1回あたりの個人資産コスト
export const EDU_GROWTH_K = 0.25;   // 教育レベル1につき子の成長係数(env.factor)を+0.25
export const CHILD_GROW_ENV = 1.1;  // 子の毎ターン基礎成長env（教育なしでもゆっくり育つ）

/* ------------------------------------------------------------
 * v0.14/0.15：個人資産の収支・結婚市場・配偶者インカム
 *  v0.15：PC報酬・配偶者インカムを社員給与テーブル(BASE_SALARY_BY_JOB/effectiveSalary)から
 *  一貫して算出（マジックナンバー廃止）。PC＝managerスケール×会社の支払い能力、配偶者＝職種スケール。
 * ------------------------------------------------------------ */
// PC役員報酬＝ manager 給与テーブル(PC.CA) × 会社の支払い能力係数(0-1)。会社CASHから支出。
// 支払い能力：序盤の小さい会社は控えめ（founderが薄給）、CASH/評判/規模の成長で満額に近づく。
export const AFFORD_CASH_REF = 400000; // このCASHで cash項が満点
export const AFFORD_REP_FULL = 70;     // この評判で rep項が満点
export const AFFORD_SIZE_FULL = 15;    // この従業員数で size項が満点
export const AFFORD_W_CASH = 0.45;     // 支払い能力の重み（CASH / 評判 / 規模）
export const AFFORD_W_REP = 0.40;
export const AFFORD_W_SIZE = 0.15;
// 配偶者インカム＝ effectiveSalary(職種, CA, 忠誠, 国) × 世帯拠出率 ×(1+評判プレミアム)（→wealth）。
export const SPOUSE_CONTRIB = 0.6;      // 世帯拠出率（実給与スケールの6割を家計へ）
export const SPOUSE_REP_PREMIUM = 0.5;  // 評判プレミアム（評判100で+50%＝高評判の伴侶ほど稼ぐ）
export const LIFESTYLE_COST_BASE = 250; // 個人の生活費（×lifestyleFactor・wealthから毎ターン）
// 結婚市場（人材DBとは別の専用プール）
export const MARRIAGE_POOL_SIZE = 50;   // 常時いる結婚候補者数（評判0-100分布）
export const MARRIAGE_CHURN = 4;        // 毎ターン入れ替わる候補者数（動的＝他所で結婚/新規登場）
export const MARRIAGE_SCOUT_COST = 2000; // 候補のCA/PA・正確な評判を開示するスカウト費（wealthから）
export const MARRIAGE_MIN_AGE = 20;     // 結婚候補の下限年齢
export const MARRIAGE_MAX_AGE = 45;     // 結婚候補の上限年齢
...
/* ------------------------------------------------------------
 * v0.20：ゲーム内「年」（1ターン=1ヶ月・12ターン/年）。Excelタイムライン起点=1980。
 * ------------------------------------------------------------ */
